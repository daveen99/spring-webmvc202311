...
class ScoreRepositoryImplTest {
    ...
    void findAllTest() {
        // GWT 패턴
        // given: 테스트를 위해 주어지는 데이터 (Parameter) 없으면 생략

        // when: 테스트 해봐야 할 상황
        List<Score> scoreList = repository.findAll();

        // then: 테스트 결과 단언 (결과 확인) scoreList의 size가 3개라고 단언한다.
        scoreList.forEach(System.out::println);
        assertEquals(3, scoreList.size());
        assertNotNull(scoreList);
        assertEquals("김흥국", scoreList.get(0).getName());
    }

    @Test
    @DisplayName("저장소에서 findOne을 호출하여 학번이 2인 학생을 조회하면" +
            "그 학생의 국어점수가 33점이고 이름은 보이비여야 한다.")
    void findOneTest() {
        //given
        int stuNum = 2;
        //when
        ...
        assertEquals("보이비", score.getName());
    }

    @Test
    @DisplayName("학번이 -99번인 학생을 조회하면 Null이 나와야만 한다.")
    ...
    void deleteTest() {
        ...
        List<Score> scoreList = repository.findAll();
        Score score2 = repository.findOne(stuNum);
        // then
    ...
    void saveTest() {
        //given
        Score s4 = new Score("지드래곤", 99, 76, 70, 4, 0, 0.0, Grade.D);
        //when
        repository.save(s4);
        List<Score> scoreList = repository.findAll();
        //then
