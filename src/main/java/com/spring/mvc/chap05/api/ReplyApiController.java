 ...
 * => /replies/write          (x) - 글 작성
 * => /replies     : POST 방식 (o) - 글 작성
 *
 * => /replies/all           (x) - 전체조회
 * => /replies     : GET 방식 (o) - 전체조회
 *
 * => /replies/17            (x) - 단일조회
 * => /replies     : GET 방식 (o) - 단일조회
 *
 * => /replies/delte?replyNo=3  (x)
 * => /replies/3      : DELETE 방식  (o)
 */
...
public class ReplyApiController {
    ...
    public ResponseEntity<?> update(
        ...
        try {
            ...
            log.warn("internal server error!! casues by : {}", e.getMessage());
            return ResponseEntity.internalServerError().body(e.getMessage());
        }
    }
