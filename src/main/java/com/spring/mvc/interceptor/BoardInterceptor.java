...
public class BoardInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(
        ...
        // 삭제요청이 들어올 때 서버에서 다시한번 관리자 or 작성자가 삭제하는것인지 체크

        // 현재요청이 삭제요청인지 확인
        String uri = request.getRequestURI();
        if (uri.contains("delete")) {
            // 글번호는 어떻게 구함? - 쿼리스트림에서 구함 ? 뒤에 붙어있음
            String bno = request.getParameter("bno");

            // 로그인한 계정명과 게시물의 계정명이 일치하는지 체크
            // ?? 로그인한 계정명은 어떻게 구합니까? -> 세션에서 가져옵니다.
            // ?? 게시물 계정명은 어떻게 구합니까? -> DB에서 가져옵니다.
            String targetAccount = boardMapper.findOne(Integer.parseInt(bno)).getAccount();

            // 만약에 관리자라면 --> 삭제 통과 !
            if (isAdmin(session)) return true;
            // 만약에 내가 쓴 글이 아니면 --> 접근권한이 없다는 안내 페이지로 이동시킨다.
